...
class ChatService {
  ...
  /**
   * Searches for Practice users by email or name.
   * Only users with practitionerType 'Practice' will be returned.
  ...
  async searchUsers(query: string): Promise<ChatUser[]> {
    try {
      const response = await fetch(
        `${this.baseUrl}/chat/users/search?query=${query}&practitionerType=Practice`,
        {
            ...
            ? {
                ...
                timestamp: normalizeDate(
                  thread.lastMessage.timestamp ||
                    (thread.lastMessage as any).createdAt,
                ),
              }
            : undefined,
        }));
  ...
  async getMessages(recipientId: string): Promise<ChatMessage[]> {
    try {
      const response = await fetch(
        `${this.baseUrl}/chat/messages/${recipientId}`,
        {
          headers: this.getHeaders(),
        },
      );
